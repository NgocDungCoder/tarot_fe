...
class ApiClient extends IApiClient {
  ...
  Future<ProductEntity?> getProductDetailById(String productId) async {
    final res = await request(ApiMethod.get, '/products/$productId');

    return await parseJsonUtil(
      res,
      (dynamic json) => ApiResponseEntity<ProductEntity>.fromJson(
        json,
  ...
  Future<PagingResEntity<CartEntity>?> getCartOfUser({
    ...
    final apiResponse = await parseJsonUtil(
      res,
          (dynamic json) =>
      ApiResponseEntity<PagingResEntity<CartEntity>>.fromJson(
        json,
            (dynamic data) =>
        PagingResEntity<CartEntity>.fromJson(
          data,
  ...
  Future<PagingResEntity<ProductEntity>?> getProducts() async {
